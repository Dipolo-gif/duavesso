...
function wirePoseHovers(){if(typeof matchMedia==='function'&&matchMedia('(prefers-reduced-motion: reduce)').matches)return;$$('.poses').forEach(v=>{if(v.dataset.posed)return;const poses=$$('.pose',v);if(poses.length<2)return;v.dataset.posed='1';let idx=0,timer=null;const show=i=>poses.forEach((el,k)=>el.classList.toggle('is-active',k===i));const stop=()=>{clearInterval(timer);timer=null;idx=0;show(0);};const start=()=>{if(timer)return;timer=setInterval(()=>{idx=(idx+1)%poses.length;show(idx);},760);};const host=v.closest('.product-image-button')||v.closest('.brand-prod')||v;host.addEventListener('mouseenter',start);host.addEventListener('mouseleave',stop);host.addEventListener('focusin',start);host.addEventListener('focusout',stop);});}
...
$('#marcas-view').addEventListener('click',e=>{const b=e.target.closest('[data-gallery]');if(!b)return;const prod=BRANDS.flatMap(x=>x.products||[]).find(p=>p.id===b.dataset.gallery);if(prod)showGallery(prod);});
...
const BRANDS=[
 {slug:'geek',name:'duavessogeek',theme:'geek',kicker:'games · pixel · sci-fi',logo:'<span class="brand-logo-word">duavesso</span><b class="brand-logo-tag">geek</b>',lead:'Cultura geek no avesso: games, pixel, sci-fi e as referências que só quem é do meio pega, no caimento oversized da duavesso.',art:{src:'geek-invader.svg',w:264,h:192},site:null,collection:'Primeiros drops',products:[{id:'geek-coracao',name:'Coração Pixelado',type:'Oversized preta',photos:['geek-coracao-1','geek-coracao-2','geek-coracao-3'],desc:'Coração anatômico em pixel art que se desmancha em blocos, com PLAYER 01 no peito. Oversized preta em suedine 250 g.'},{id:'geek-carpa',name:'Carpa Japonesa',type:'Oversized off white',photos:['geek-carpa-1','geek-carpa-2','geek-carpa-3'],desc:'Carpa koi em nanquim com o sol laranja, entre a arte japonesa e o traço geek. Oversized off white em suedine 250 g.'}],drops:[['&lt;3','Pixel Heart'],['1UP','Continue?'],['404','Not Found']]},
...
function brandCollection(brand){
 ...
 const cards=brand.products.map(p=>p.photos?`<button type="button" class="brand-prod brand-prod-gallery" data-gallery="${p.id}"><div class="brand-prod-img poses">${p.photos.map((n,i)=>`<div class="pose${i?'':' is-active'}">${photoPicture(n,`${esc(p.name)}, foto ${i+1}`,'(max-width:700px) 45vw, 22vw','loading="lazy"')}</div>`).join('')}</div><div class="brand-prod-meta"><span class="brand-prod-name">${esc(p.name)}</span>${p.type?`<span class="brand-prod-type">${esc(p.type)}</span>`:''}</div></button>`:`<a class="brand-prod"${p.href?` href="${p.href}"`:brand.site?` href="${brand.site}"`:''} target="_blank" rel="noopener">${p.img?`<div class="brand-prod-img"><picture><source type="image/webp" srcset="assets/${p.img}.webp"><img src="assets/${p.img}.jpg" alt="${esc(p.name)}" width="800" height="800" loading="lazy"></picture></div>`:`<div class="brand-prod-tile"><span>${esc(p.name)}</span></div>`}<div class="brand-prod-meta"><span class="brand-prod-name">${esc(p.name)}</span>${p.type?`<span class="brand-prod-type">${p.type}</span>`:''}${p.price?`<span class="brand-prod-price">${p.price}</span>`:''}</div></a>`).join('');
...
function showGallery(prod){
 let pose=0;const root=$('#product-detail');
 root.innerHTML=`<div class="detail-layout"><div class="detail-gallery"><div class="detail-main" id="detail-main"></div><div class="detail-thumbs" id="detail-thumbs" role="group" aria-label="Fotos do produto"></div></div><div class="detail-copy"><span class="eyebrow">DUAVESSOGEEK</span><h2>${esc(prod.name)}</h2>${prod.type?`<p class="detail-color">${esc(prod.type)}</p>`:''}${prod.desc?`<p>${esc(prod.desc)}</p>`:''}<p class="helper">Primeiros drops da duavessogeek. Em breve na loja.</p></div></div>`;
 const mainEl=$('#detail-main',root),thumbsEl=$('#detail-thumbs',root);
 const render=()=>{mainEl.innerHTML=photoPicture(prod.photos[pose],`${esc(prod.name)}, foto ${pose+1} de ${prod.photos.length}`,'(max-width:700px) 96vw, 560px');thumbsEl.innerHTML=prod.photos.map((n,i)=>`<button type="button" class="detail-thumb${i===pose?' active':''}" data-pose="${i}" aria-pressed="${i===pose}"><span class="sr-only">Foto ${i+1}</span>${photoPicture(n,'','80px','loading="lazy"')}</button>`).join('');};
 render();
 thumbsEl.addEventListener('click',e=>{const b=e.target.closest('[data-pose]');if(!b)return;pose=+b.dataset.pose;render();});
 openDialog('#product-dialog');
}
