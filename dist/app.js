...
const BRANDS=[
 ...
 {slug:'try84',name:'TRY84',theme:'rugby',kicker:'rugby lifestyle · forward together',logo:'<span class="brand-logo-word">TRY84</span>',lead:'Feita por jogadores. Rugby lifestyle em preto e branco, do treino ao terceiro tempo, com o caimento oversized da duavesso.',art:{src:'brand-try84.svg',w:280,h:180},site:'https://try84.com.br',drops:[['84','Kickoff'],['TRY','Try Line'],['III','3º Tempo']]},
 {slug:'solfado',name:'SolFáDó',theme:'music',kicker:'camisetas inspiradas na música',logo:'<span class="brand-logo-word">SolFáDó</span>',lead:'Música é a arte do som. Estampas que traduzem ritmo, clave e melodia, no caimento oversized da duavesso.',art:{src:'brand-solfado.svg',w:280,h:170},site:'https://solfado.com.br',drops:[['Sol','Clave de Sol'],['Fá','Compasso'],['Dó','Escala']]},
];
function renderMarcas(slug){
 ...
 view.innerHTML=`<div class="editor-heading"><a href="#marcas" class="text-link">← Todas as marcas</a><span class="eyebrow">MARCAS · ${brand.name}</span></div><article class="brand" data-theme="${brand.theme}"><div class="brand-hero"><div class="brand-hero-copy"><span class="brand-kicker">${brand.kicker}</span><div class="brand-logo">${brand.logo}${brand.theme==='geek'?'<span class="brand-cursor" aria-hidden="true"></span>':''}</div><p class="brand-lead">${brand.lead}</p><div class="brand-cta"><a href="#colecao" class="button button-blue">Ver a base oversized <span>↗</span></a>${brand.site?`<a class="brand-site" href="${brand.site}" target="_blank" rel="noopener">${brand.site.replace('https://','')} ↗</a>`:''}<span class="brand-soon">drops: <b>em breve</b></span></div></div><div class="brand-hero-art"><img src="assets/${brand.art.src}" alt="" width="${brand.art.w}" height="${brand.art.h}" loading="lazy"></div></div><div class="brand-drops"><div class="section-heading"><h2>Primeiros drops</h2><span class="brand-kicker">em produção</span></div><div class="brand-grid">${brand.drops.map(d=>`<div class="brand-drop"><span class="brand-drop-art">${d[0]}</span><span class="brand-drop-name">${d[1]}</span><span class="brand-drop-soon">EM BREVE</span></div>`).join('')}</div></div><form class="brand-notify"><label>Avise-me quando a ${brand.name} lançar.</label><div class="brand-notify-row"><input type="text" name="website" class="hp" tabindex="-1" autocomplete="off" aria-hidden="true"><input type="email" name="email" placeholder="seu@email.com" required maxlength="120" autocomplete="off"><button type="submit" class="button button-blue">Quero</button></div></form></article>`;
}
function route(){
