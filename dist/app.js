...
function renderCatalog(){
 ...
 let products=PRODUCTS.filter(p=>!p.brand&&(filter==='all'||p.category===filter)&&`${p.name} ${p.color}`.toLocaleLowerCase('pt-BR').includes(query));
...
$('#marcas-view').addEventListener('click',e=>{const b=e.target.closest('[data-product]');if(b)showProduct(b.dataset.product);});
...
function brandCollection(brand){
 ...
 if(own.length){
  const cards=own.map(p=>`<button type="button" class="brand-prod brand-prod-gallery" data-product="${p.id}"><div class="brand-prod-img poses">${p.photos.map((n,i)=>`<div class="pose${i?'':' is-active'}">${photoPicture(n,`${esc(p.name)}, foto ${i+1}`,'(max-width:700px) 45vw, 22vw','loading="lazy"')}</div>`).join('')}</div><div class="brand-prod-meta"><span class="brand-prod-name">${esc(p.name)}</span><span class="brand-prod-type">${esc(p.color)}</span><span class="brand-prod-price">${money(p.price)}</span></div></button>`).join('');
